...
import { Eye, EyeOff, AlertTriangle, CheckCircle, Sparkles } from 'lucide-react';
...
export default function Login({ setCurrentPage, redirectAfterLogin }: LoginProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    setLoading(false);
    if (result.success) {
      setCurrentPage(redirectAfterLogin || 'member');
        ...
        {!showAdminForm ? (
          <form onSubmit={handleGoogleLogin} className="space-y-4">
            ...
            <Button
              ...
              loading={loading}
              icon={Sparkles}
              className="w-full justify-center gap-2 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50"
              onClick={() => {
                handleGoogleLogin();
              }}
            >
